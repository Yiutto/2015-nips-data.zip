...
for link in paper_links:
    ...
    fulltext = text_from_pdf(pdf_path, temp_path)
    print(paper_title)
    papers.append([paper_id, paper_title, event_type, pdf_name, abstract, fulltext])

pd.DataFrame(list(nips_authors), columns=["Id","Name"]).to_csv("output/Authors.csv", index=False)
pd.DataFrame(papers, columns=["Id", "Title", "EventType", "PdfName", "Abstract", "Fulltext"]).to_csv("output/Papers.csv", index=False)
